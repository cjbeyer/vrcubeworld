...
bool ovrVkDevice_SelectPhysicalDevice( ovrVkDevice * device, ovrVkInstance * instance, const char * requiredExtensionNames, uint32_t requiredExtensionNamesSize )
{
	...
	for ( uint32_t physicalDeviceIndex = 0; physicalDeviceIndex < physicalDeviceCount; physicalDeviceIndex++ )
	{
		...
		if ( device->supportsMultiview )
		{
			...
			deviceMultiviewFeatures.multiview = 0;
			deviceMultiviewFeatures.multiviewGeometryShader = 0;
			deviceMultiviewFeatures.multiviewTessellationShader = 0;
		...
		if ( device->supportsMultiview )
		{
			ALOGV( "Device %s multiview rendering, with %d views and %d max instances", deviceMultiviewFeatures.multiview ? "supports" : "does not support",
